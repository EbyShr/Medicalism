...
window.ComponentRenderer = {
  ...
  renderSection(section, chapterId) {
    ...
    if (chapterId === 'ch-01') {
      ...
      sectionBody = this.renderChapter21Section(section);
  ...
  /* =========================================================================
     CHAPTER 21 RENDERERS (ANTIMICROBIAL RESISTANCE - AMR)
     ========================================================================= */
  renderChapter21Section(sec) {
    if (!sec) return '';

    if (sec.id === 'ch21-sec01') {
      const defs = sec.definitions || [];
      const dss = sec.dominantStrainSelection || {};
      const outcomes = sec.clinicalAndEconomicOutcomes || [];

      const defCards = defs.map(d => `
        <div class="stage-step-card">
          <div class="step-num-badge">تعریف</div>
          <div style="flex: 1;">
            <strong style="color: var(--text-primary); display: block; margin-block-end: 4px;">${d.term || ''}</strong>
            <span style="font-size: var(--font-size-sm); color: var(--text-secondary); line-height: var(--line-height-relaxed);">${d.desc || ''}</span>
          </div>
        </div>
      `).join('');

      const outCards = outcomes.map((o, idx) => `
        <div class="key-fact-card" style="border-inline-start: 4px solid var(--state-danger);">
          <div class="card-label">پیامد ${idx + 1}</div>
          <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${o}</p>
        </div>
      `).join('');
      ...
      return `
        ...
        <h3 style="margin-block-end: var(--space-3);">تعاریف پایه و مفاهیم مقاومت میکروبی</h3>
        <div class="grid-2" style="margin-block-end: var(--space-6);">${defCards}</div>

        <div class="clinical-card" style="margin-block-end: var(--space-6);">
          <div class="card-header bg-danger text-white d-flex align-center gap-2">
            <span class="icon">🧬</span><strong>دینامیک انتخاب سویه غالب و قانون مقاومت متقاطع</strong>
          </div>
          <div class="card-body">
            <p style="margin-block-end: var(--space-3); line-height: var(--line-height-relaxed);">${dss.pathway || ''}</p>
            <div style="background: var(--color-surface-subtle); padding: 12px 16px; border-radius: var(--radius-sm); border: 1.5px solid var(--border-default);">
              <strong style="color: var(--state-danger);">قانون مقاومت متقاطع (Cross-Resistance):</strong>
              <p style="margin: 4px 0 0 0; font-size: var(--font-size-sm);">${dss.crossResistanceRule || ''}</p>
            </div>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">پیامدهای بالینی و اقتصادی مقاومت میکروبی</h3>
        <div class="grid-2">${outCards}</div>
      `;
    }

    if (sec.id === 'ch21-sec02') {
      const hc = sec.humanAndClinicalDeterminants || {};
      const env = sec.environmentalAndSocialFactors || [];
      const vet = sec.veterinaryCrisis || {};
      const hazards = vet.fourfoldHazards || [];

      const envCards = env.map(e => `
        <div class="clinical-card">
          <div class="card-header" style="background: var(--color-surface-subtle); color: var(--text-primary); border-bottom: 1px solid var(--border-subtle);">
            <span class="hazard-badge" style="margin: 0;">عامل تعیین‌کننده جمعیتی-محیطی</span>
          </div>
          <div class="card-body">
            <strong style="display: block; margin-block-end: 6px; font-size: 1.05rem;">${e.factor || ''}</strong>
            <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary); line-height: var(--line-height-relaxed);">${e.desc || ''}</p>
          </div>
        </div>
      `).join('');

      const hazardClasses = ['hazard-health', 'hazard-food', 'hazard-env', 'hazard-economy'];
      const hazardBadges = [
        '<span class="hazard-badge">💊 بهداشتی و انسانی</span>',
        '<span class="hazard-badge">🍗 صنایع غذایی و سلامت غذا</span>',
        '<span class="hazard-badge">🌱 زیست‌محیطی</span>',
        '<span class="hazard-badge">📉 اقتصاد ملی</span>'
      ];

      const hazardCards = hazards.map((h, idx) => `
        <div class="hazard-card ${hazardClasses[idx % 4]}">
          ${hazardBadges[idx % 4]}
          <p style="margin: 8px 0 0 0; font-size: var(--font-size-sm); line-height: var(--line-height-relaxed);">${h.impact || h.desc || ''}</p>
        </div>
      `).join('');
      ...
      return `
        ...
        <div class="grid-2" style="margin-block-end: var(--space-6);">
          <div class="clinical-card">
            <div class="card-header bg-primary text-white d-flex align-center gap-2">
              <span class="icon">💊</span><strong>طب انسانی و رفتار مصرف‌کنندگان</strong>
            </div>
            <div class="card-body">
              <p style="margin: 0; line-height: var(--line-height-relaxed);">${hc.prescribingAndPatient || ''}</p>
            </div>
          </div>
          <div class="clinical-card">
            <div class="card-header bg-danger text-white d-flex align-center gap-2">
              <span class="icon">🏥</span><strong>بیمارستان‌ها: کانون اصلی مقاومت در دنیا</strong>
            </div>
            <div class="card-body">
              <p style="margin: 0; line-height: var(--line-height-relaxed);">${hc.hospitalHotspot || ''}</p>
            </div>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">عوامل محیطی، جمعیتی و اجتماعی</h3>
        <div class="grid-3" style="margin-block-end: var(--space-6);">${envCards}</div>

        <div class="clinical-card" style="margin-block-end: var(--space-6);">
          <div class="card-header bg-danger text-white d-flex align-center gap-2">
            <span class="icon">⚠️</span><strong>بحران دامپزشکی و مصرف ضدمیکروب‌ها در دامپروری</strong>
          </div>
          <div class="card-body">
            <div class="grid-2" style="margin-block-end: var(--space-4);">
              <div class="key-fact-card">
                <div class="card-label">بیانیه مرجع FDA آمریکا</div>
                <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${vet.fdaStatement || ''}</p>
              </div>
              <div class="key-fact-card" style="border-inline-start: 4px solid var(--state-danger);">
                <div class="card-label">سهم مصرف در اتحادیه اروپا</div>
                <p style="margin: 0; font-size: var(--font-size-sm); font-weight: 700; color: var(--state-danger);">${vet.livestockShare || ''}</p>
              </div>
            </div>
            <p style="margin-block-end: var(--space-2); font-size: var(--font-size-sm);"><strong>موارد مصرف غیردرمانی:</strong> ${vet.nonTherapeuticUses || ''}</p>
            <p style="margin: 0; font-size: var(--font-size-sm);"><strong>ارگانیسم‌های درگیر:</strong> ${vet.affectedPathogens || ''}</p>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">مخاطرات چهارگانه سوءمصرف آنتی‌بیوتیک در دام</h3>
        <div class="grid-3" style="margin-block-end: var(--space-6);">${hazardCards}</div>

        <div class="historical-milestone-card" style="border-inline-start-color: var(--color-primary);">
          <div class="historical-milestone-header">
            <span class="icon">🌐</span><strong>موضع رسمی سازمان جهانی بهداشت (WHO)</strong>
          </div>
          <div class="historical-milestone-body">
            <p style="margin: 0; font-size: 1rem;">${vet.whoBanPolicy || ''}</p>
          </div>
        </div>
      `;
    }

    if (sec.id === 'ch21-sec03') {
      const bio = sec.biologicalMechanisms || {};
      const crit = sec.criticalPathogensChallenges || {};
      const priorities = sec.who12BacterialPriorities || [];
      const nonBact = sec.nonBacterialResistance || {};

      const prioCards = priorities.map(p => {
        let badgeClass = 'bg-danger';
        if (p.tier.includes('بالا')) badgeClass = 'bg-warning';
        if (p.tier.includes('متوسط')) badgeClass = 'bg-info';

        return `
          <div class="clinical-card">
            <div class="card-header ${badgeClass} text-white d-flex justify-between align-center">
              <strong>${p.tier || ''}</strong>
              <span class="badge badge-subtle" style="background: rgba(255,255,255,0.25); color: #fff;">WHO Priority</span>
            </div>
            <div class="card-body">
              <strong style="display: block; margin-block-end: 4px;">${p.bacteria || ''}</strong>
              <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${p.resistance || ''}</p>
            </div>
          </div>
        `;
      }).join('');
      ...
      return `
        ...
        <h3 style="margin-block-end: var(--space-3);">مکانیسم‌های بیولوژیک تکامل و انتقال مقاومت</h3>
        <div class="grid-2" style="margin-block-end: var(--space-6);">
          <div class="key-fact-card">
            <div class="card-label">انتخاب داروینی (Darwinian Selection)</div>
            <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${bio.darwinianSelection || ''}</p>
          </div>
          <div class="key-fact-card">
            <div class="card-label">انتقال افقی ژن‌ها (HGT)</div>
            <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${bio.hgt || ''}</p>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">چالش‌های پاتوژن‌های فوق‌مقاوم و بحران خط تولید دارو</h3>
        <div class="clinical-card" style="margin-block-end: var(--space-6);">
          <div class="card-body">
            <p style="margin-block-end: var(--space-2);"><strong>پاتوژن‌های پیچیده:</strong> ${crit.complexPathogens || ''}</p>
            <p style="margin: 0; color: var(--state-danger);"><strong>وابستگی به آنتی‌بیوتیک‌های جدید:</strong> ${crit.newDrugsDependency || ''}</p>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">رده‌بندی ۱۲ باکتری اولویت‌دار سازمان جهانی بهداشت (WHO)</h3>
        <div class="grid-3" style="margin-block-end: var(--space-6);">${prioCards}</div>

        <h3 style="margin-block-end: var(--space-3);">مقاومت دارویی در پاتوژن‌های غیرباکتریایی</h3>
        <div class="grid-3">
          <div class="hazard-card hazard-health">
            <div class="hazard-badge">ویروس‌ها (Viruses)</div>
            <p style="margin: 0; font-size: var(--font-size-sm);">${nonBact.viruses || ''}</p>
          </div>
          <div class="hazard-card hazard-food">
            <div class="hazard-badge">انگل‌ها (Parasites)</div>
            <p style="margin: 0; font-size: var(--font-size-sm);">${nonBact.parasites || ''}</p>
          </div>
          <div class="hazard-card hazard-env">
            <div class="hazard-badge">نماتودها و کرم‌ها</div>
            <p style="margin: 0; font-size: var(--font-size-sm);">${nonBact.nematodes || ''}</p>
          </div>
        </div>
      `;
    }

    if (sec.id === 'ch21-sec04') {
      const drug = sec.iranDrugConsumption || {};
      const res = sec.iranPathogenResistancePatterns || {};
      const strat = sec.containmentStrategies || [];

      const stratCards = strat.map((s, idx) => `
        <div class="stage-step-card">
          <div class="step-num-badge">${idx + 1}</div>
          <div style="flex: 1;">
            <strong style="color: var(--text-primary); display: block; margin-block-end: 4px;">${s.axis || ''}</strong>
            <span style="font-size: var(--font-size-sm); color: var(--text-secondary);">${s.desc || ''}</span>
          </div>
        </div>
      `).join('');
      ...
      return `
        ...
        <h3 style="margin-block-end: var(--space-3);">الگوی مصرف آنتی‌بیوتیک در ایران</h3>
        <div class="grid-2" style="margin-block-end: var(--space-6);">
          <div class="key-fact-card">
            <div class="card-label">پرمصرف‌ترین داروهای خوراکی</div>
            <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${drug.topOral || ''}</p>
          </div>
          <div class="key-fact-card">
            <div class="card-label">پرمصرف‌ترین داروهای تزریقی بیمارستانی</div>
            <p style="margin: 0; font-size: var(--font-size-sm); color: var(--text-secondary);">${drug.topIv || ''}</p>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">الگوهای مقاومت پاتوژن‌ها در ایران</h3>
        <div class="grid-3" style="margin-block-end: var(--space-6);">
          <div class="clinical-card">
            <div class="card-header bg-danger text-white"><strong>باکتری‌های مقاوم در ایران</strong></div>
            <div class="card-body"><p style="margin: 0; font-size: var(--font-size-sm);">${res.bacteria || ''}</p></div>
          </div>
          <div class="clinical-card">
            <div class="card-header bg-warning text-white"><strong>ویروس‌ها و انگل‌ها</strong></div>
            <div class="card-body">
              <p style="margin-block-end: var(--space-1); font-size: var(--font-size-sm);"><strong>ویروس‌ها:</strong> ${res.viruses || ''}</p>
              <p style="margin: 0; font-size: var(--font-size-sm);"><strong>انگل‌ها:</strong> ${res.parasites || ''}</p>
            </div>
          </div>
          <div class="clinical-card">
            <div class="card-header bg-primary text-white"><strong>قارچ‌ها و دام و طیور</strong></div>
            <div class="card-body">
              <p style="margin-block-end: var(--space-1); font-size: var(--font-size-sm);"><strong>قارچ‌ها:</strong> ${res.fungi || ''}</p>
              <p style="margin: 0; font-size: var(--font-size-sm);"><strong>دام و طیور:</strong> ${res.livestockAndPoultry || ''}</p>
            </div>
          </div>
        </div>

        <h3 style="margin-block-end: var(--space-3);">راهبردهای چهارگانه مهار و اقدام ملی</h3>
        <div class="grid-2">${stratCards}</div>
      `;
    }
